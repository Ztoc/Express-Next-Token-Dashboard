...
import type { DerivedPairDataNormalized, PairDataNormalized } from "./types";
  ...
  builder
    ...
    .addCase(
      ...
      (state, { payload: { pairData, pairId, timeWindow } }) => {
        if (!(state?.pairDataById as any)[pairId]) {
          (state.pairDataById as any)[pairId] = {};
        }
        (state.pairDataById as any)[pairId][timeWindow] = pairData;
      }
    )
    .addCase(
      updateDerivedPairData,
      (state, { payload: { pairData, pairId, timeWindow } }) => {
        if (!(state.derivedPairDataById as any)[pairId]) {
          (state.derivedPairDataById as any)[pairId] = {};
        }
        (state.derivedPairDataById as any)[pairId][timeWindow] = pairData;
      }
    )
);
