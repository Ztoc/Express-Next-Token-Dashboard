...
export const normalizeChartData = (
  ...
  switch (timeWindow) {
    ...
    case PairDataTimeWindowEnum.WEEK:
      return (data as PairHoursDatasResponse)?.pairHourDatas?.map(
        (fetchPairEntry) => ({
    ...
    case PairDataTimeWindowEnum.YEAR:
      return (data as PairDayDatasResponse)?.pairDayDatas?.map(
        (fetchPairEntry) => ({
...
export const normalizeDerivedChartData = (data: any) => {
  ...
  const token1DerivedUSDEntryMap: any = fromPairs(
    data?.token1DerivedUSD?.map((entry: any) => [entry.timestamp, entry]) ?? []
  );

  return data?.token0DerivedUSD.reduce(
    (acc: any, token0DerivedUSDEntry: any) => {
      const token1DerivedUSDEntry =
        token1DerivedUSDEntryMap[token0DerivedUSDEntry.timestamp];
