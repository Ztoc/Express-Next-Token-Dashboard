...
import type { PairDayDatasResponse, PairHoursDatasResponse } from "./types";
...
export const getIdsByTimeWindow = ({
  ...
  return times(
    ...
    (value) =>
      `${pairAddress}-${
        pairLastIdAsNumber - value * (timeWindowGapMapping as any)[timeWindow]
      }`
  );
};
...
export const pairHasEnoughLiquidity = (
  ...
  switch (timeWindow) {
    ...
    case PairDataTimeWindowEnum.WEEK: {
      const amountOfDataPoints =
        (data as PairHoursDatasResponse)?.pairHourDatas?.length ?? 1;
    ...
    case PairDataTimeWindowEnum.YEAR: {
      const amountOfDataPoints =
        (data as PairDayDatasResponse)?.pairDayDatas?.length ?? 1;
